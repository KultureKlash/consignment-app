...
const statusColors: Record<string, { bg: string; color: string }> = {
  [ORDER_STATUS.OPEN]: { bg: "#ecfdf5", color: "#059669" },
  [ORDER_STATUS.CANCELLED]: { bg: "#fef2f2", color: "#dc2626" },
  [ORDER_STATUS.REFUNDED]: { bg: "#fffbeb", color: "#d97706" },
        ...
        <div className="admin-card mb-6">
          ...
          <table className="hidden md:table w-full border-collapse text-[13px]">
            ...
            {order.items.map((item, i) => {
              ...
              const saleTx = item.transactions.find((tx) => tx.type === TRANSACTION_TYPE.SALE);
              const isLast = i === order.items.length - 1;
              const isPending = order.paymentStatus === ORDER_PAYMENT_STATUS.PENDING && item.status === LISTING_STATUS.SOLD;
